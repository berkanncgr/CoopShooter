...
void ASTrackerBot::BeginPlay()
{
	...
	if (HasAuthority())
	{
		...
		// Every second we update our power-level based on nearby bots. If there is a another bot nearby, damage increases.
...
FVector ASTrackerBot::GetNextPathPoint()
{
	...
	if (BestTarget)
	{
		...
		// Look for best target again after 5 seconds.
...
void ASTrackerBot::NotifyActorBeginOverlap(AActor* OtherActor)
{
	...
	if (!bStartedSelfDestruction && !bExploded)
	{
		...
		if (PlayerPawn && !USHealthComponent::IsFriendly(OtherActor, this))
		{
			...
			if (HasAuthority())
			{
				// Start self destruction sequence and explode
...
// If there is a another bot, increase damage.
void ASTrackerBot::OnCheckNearbyBots()
{
	...
	TArray<FOverlapResult> Overlaps; // Out param
